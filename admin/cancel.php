  ...
  <body>
    ...
    <div id="wrapper">
      ...
      <div id="page-wrapper">
        ...
        <div class="row">
          <div class="col-lg-12">
            <h1>Request<small>cancelled</small></h1>
           <hr>
          </div>
        </div><!-- /.row --><!-- /.row -->
...
if (isset($_POST['statupdate'])){
    ...
    if (mysql_num_rows($txnquery)==0){
		...
		if($status=='Cancelled'){
				...
				$product_id_string = $product_id_array;
...
<tbody>
  <?php
  $txn="";
  $cancel = mysql_query("SELECT * FROM cacel_order ORDER BY id DESC");
if ($cancel > 0) {
	while($row = mysql_fetch_array($cancel)){ 
			 $id = $row['id'];
			 $txn = $row['txn_id'];
			 
			 $sql = mysql_query("SELECT * FROM transactions WHERE txn_id='$txn' ORDER BY id DESC");
